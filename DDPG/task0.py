...
from chargeenv import ChargeEnv as CE
...
class Config:
    ...
    def __init__(self):
        ...
        self.memory_capacity = 51200  # 经验回放的容量
...
def train(cfg, env, agent):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while not done:
            ...
            position.append([next_state[80], next_state[81]])
            # print(action[0],reward)
            ep_reward += reward
        ...
        if min_step > i_step:
            min_step = i_step
            np.save('position.npy',np.array(position))
...
if __name__ == "__main__":
    ...
    env = CE()
    # 训练
    agent = env_agent_config(cfg, seed=1)
    rewards, ma_rewards = train(cfg, env, agent)
    make_dir(cfg.result_path, cfg.model_path)
    agent.save(path=cfg.model_path)
    save_results(rewards, ma_rewards, tag='train', path=cfg.result_path)
    plot_rewards(np.array(rewards), np.array(ma_rewards), cfg, tag="train")  # 画出结果
    # 测试
    agent = env_agent_config(cfg, seed=10)
    agent.load(path=cfg.model_path)
    rewards, ma_rewards = test(cfg, env, agent)
    save_results(rewards, ma_rewards, tag='test', path=cfg.result_path)
    plot_rewards(np.array(rewards), np.array(ma_rewards), cfg, tag="test")  # 画出结果
